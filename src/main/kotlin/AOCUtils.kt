...
class AOCUtils {
    ...
    private fun decompressGZIP(inputStream: InputStream?): String {
        ...
        val bodyStream: InputStream = try {
            ...
            println("Error decompressing GZIP")
            inputStream!!
        }
    ...
    fun getInputLines(): List<String> {
        ...
        if(cookie.isNullOrBlank()){
            ...
            println("AOC_COOKIE found: ${cookie}...")
        }
...
//    println(response.headers())
    ...
    private fun saveOutput(s: String, nDay: Int, level: String) = File("src", "outputs/$nYear-$nDay-$level-${Instant.now().toString().replace(":", "_")}.txt")
        .writeText(s)
